...
function userProfile( $option, $uid, $submitvalue) {
  ...
  // Place projects code here!!!!
  $profid= $_GET['user'];
  if(empty($profid)) $profid=$uid;
  $db = JFactory::getDBO();
  $query = $db->getQuery (true);
  $query->select(array('subject, id, positions, thread'));
  $query->from('n2ns0_kunena_messages');
  $query->where('userid = '.$profid);
  $query->order('id DESC');
  $db->setQuery($query);
  $row = $db->loadRowList();
  if($row){
    echo "<p style='font-size:30px;'><b>Current Projects:</b></p>";
    echo"<table><tr style='font-weight:bold;background-color:black;color:white'><td style='width:50px;'>Status</td><td style='width:700px'>Project</td><td style='width:250px'>Positions Needed</td></tr>";
    for ( $i = 0; $i < sizeof($row); $i++ ) {
      echo "<tr>";
      $mesquery = "SELECT icon_id from n2ns0_kunena_topics WHERE id=".$row[$i][3];
      $db->setQuery($mesquery);
      $iconstatus = $db->loadResult();
      
      echo "<td style='vertical-align:middle;'>";
      switch($iconstatus){
        case 0:
          echo "<img src='/media/kunena/topicicons/default/user/PRE-PRODUCTION.png'title='Pre-Production'/>";
          break;
        case 1:
          echo "<img src='/media/kunena/topicicons/default/user/PRODUCTION.png' title='In Production'/>";
          break;
        case 2:
          echo "<img src='/media/kunena/topicicons/default/user/POST-PRODUCTION.png' title='Post Production'/>";
          break;
      }
      echo "</td>";
      $mesquery = "SELECT * from n2ns0_kunena_messages_text WHERE mesid=".$row[$i][1];
      $db->setQuery($mesquery);
      $message = $db->loadRowList();
      echo "<td><b><a href='/index.php?option=com_kunena&view=topic&catid=2&id=".$row[$i][3]."&Itemid=136'>".$row[$i][0]."</a></b><br><br>".KunenaHtmlParser::stripBBCode($message[0][1],500)."</td>";
      echo "<td>".KunenaHtmlParser::stripBBCode($row[$i][2],500)."</td>";
      //$row = $db->loadRow();
      echo "</tr>";
    }
    echo "</table>";
    ...
    echo "<b>This student does not have any current projects</b>";
  }
}
...
function saveRegistration( $option ) {
  ...
  $email                 =  cbGetParam($_POST,'email','');
  $splitemail              =  explode( '@', $email );
  if($splitemail[1] == "binghamton.edu"){
    if ( $usernameExists ) {
      ...
      if ( $userComplete->verifyPassword( $password ) ) {
        ...
        $msg            =  "You must use a binghamton.edu email to register for this website";
        echo "<script type=\"text/javascript\">alert('" . addslashes( $msg ) . "'); </script>\n";
        $oldUserComplete        =  new moscomprofilerUser( $_CB_database );
        $userComplete->bindSafely( $_POST, $_CB_framework->getUi(), 'register', $oldUserComplete );
        HTML_comprofiler::registerForm( $option, $ueConfig['emailpass'], $userComplete, $_POST, htmlspecialchars( $msg ) );
