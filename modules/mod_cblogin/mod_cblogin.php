...
if ( $_CB_framework->myId() ) {
  ...
  // dont think its needed echo '<div style="width:100%; padding:20px; border-width:0px; margin:0px;">'."\n";
...
echo "<center>";
 if ( $show_avatar and ($avatar_position!="default") and ($greeting) ) {
    if ($avatarDisplayed && ( ! $horizontal ) ) {
      ...
      echo '<span id="mod_login_greeting'.$id_sfx.'">'.$cbUser->replaceUserVars(sprintf( _UE_HI_NAME, $name )).'</span><br>';
    }
  }
  if(false){
  $pms = 0;
  if($show_pms != 0) {
    ...
    if($pms != 0)
    {
      ...
      if (($total_pms) > 0 ) {
        ...
        $pmsMsg .= '<span id="mod_login_messagestext'.$id_sfx.'">'._UE_PM_MESSAGES_HAVE." ".$total_pms."&nbsp;".($total_pms == 1 ? "message" : "messages")."</span></a><br>\n";
      } else {
        if($show_pms >= 2 ) {
  ...
  if ($show_avatar == 0) {
    ...
    if ($oValue) {
      ...
      echo '<img src="' . htmlspecialchars( $oValue ) . '" style="margin-left:auto; margin-right:auto; border-width:0px;" alt="'.$name
...
*/
  ...
  echo '<br><span><input type="text" name="username" id="mod_login_username'.$id_sfx.'" class="inputbox'.$class_sfx.' input-medium" size="'.$name_lenght.'"' . $bgstyleUser . ' /></span>';
  if ($horizontal) {
    //echo "&nbsp;\n";
  } elseif ( $jVersion == 2 ) {
    echo '</p>';
  ...
  switch ($remember_enabled) {
    ...
    case 1:
    case 3:
      $remInput  =  '</br><input type="checkbox" name="remember" id="mod_login_remember'.$id_sfx.'"' . ( $class_sfx ? ' class="inputbox'.$class_sfx.'"' : '' ) . ' value="yes"' . ( $remember_enabled == 3 ? ' checked="checked"' : '' ) . ' />';
      $remLabel  =  '<div style="display: inline-block"><span  id="mod_login_remembermetext'.$id_sfx.'"><label for="mod_login_remember'.$class_sfx.'">'._UE_REMEMBER_ME."</label>  </span></div></br>";
      if ( $jVersion == 2 ) {
        if ( ! $horizontal ) {
  ...
  if ( $urlRegister ) {
    ...
    //    echo _UE_NO_ACCOUNT . " ";
    //  }
    echo '<br><a href="'.$urlRegister.'" class="mod_login'.$class_sfx.'">';
    if ( $show_remind_register_icons >= 1 ) {
